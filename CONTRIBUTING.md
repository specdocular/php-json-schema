# Contributing to PHP JSON Schema

Thanks for your interest in improving PHP JSON Schema — a type-safe, fluent PHP
implementation of JSON Schema Draft 2020-12. This guide covers how to set up the
project, the checks your change must pass, and how releases are versioned.
...
PHP JSON Schema is a framework-agnostic PHP library with no runtime framework
dependency.

```bash
git clone https://github.com/specdocular/php-json-schema.git
cd php-json-schema
composer install
```
...
PHP JSON Schema follows [Semantic Versioning 2.0.0][semver]. Releases are git tags of
the form `vMAJOR.MINOR.PATCH` (for example `v0.1.0`).
...
PHP JSON Schema is licensed under the [MIT License](LICENSE). By contributing, you
